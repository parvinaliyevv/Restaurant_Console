...
namespace food {
	class Interface {
	public:
		Interface(const string& nameP, const int& price, const int& dishCategory);
		...
		void IncreaseIngredient(const Provision productP, const short& amountP) noexcept;
		void DecreaseIngredient(const Provision productP, const short& amountP) throw(MyException);
		...
		const Provision& GetIngredient(const int& index) const noexcept;
		const uint32_t GetIngredientsCalories() const noexcept;
